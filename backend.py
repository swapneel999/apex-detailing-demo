...
# ----------------------------------------------------------------
# App setup — all secrets from environment, never hardcoded
# ----------------------------------------------------------------
app = Flask(__name__)
...
# Rate limiter — in-memory storage (swap to Redis in production)
limiter = Limiter(
    get_remote_address,
    app=app,
...
# ----------------------------------------------------------------
# Authorization decorator — IDOR/privilege-escalation prevention
# ----------------------------------------------------------------
def require_admin(f):
    @wraps(f)
...
def submit_quote():
    ...
    # Enum whitelist — prevents IDOR-class injection via package field
...
# ----------------------------------------------------------------
# Admin: List quotes — no user-supplied ID, no IDOR risk
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# Admin: Fetch single quote by opaque UUID — IDOR-safe
# ----------------------------------------------------------------
...
def admin_setup():
    ...
    # bcrypt with work factor 12 — adaptive slow hash, never MD5/SHA
